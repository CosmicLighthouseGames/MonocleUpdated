...
namespace Monocle
{
    public class Tween : Component
    {
        ...
        public static Tween Position(Entity entity, Vector3 targetPosition, float duration, Ease.Easer easer, TweenMode tweenMode = TweenMode.Oneshot)
        {
            Vector3 startPosition = entity.Position;
            Tween tween = Tween.Create(tweenMode, easer, duration, true);
            tween.OnUpdate = (t) => { entity.Position = Vector3.Lerp(startPosition, targetPosition, t.Eased); };
            entity.Add(tween);
            return tween;
        }
