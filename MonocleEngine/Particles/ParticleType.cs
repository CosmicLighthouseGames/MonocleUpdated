...
namespace Monocle
{
    public class ParticleType
    {
        ...
        public Particle Create(ref Particle particle, Vector3 position)
        {
            return Create(ref particle, position, Direction);
        }

        public Particle Create(ref Particle particle, Vector3 position, Color color)
        {
            return Create(ref particle, null, position, Direction, color);
        }

        public Particle Create(ref Particle particle, Vector3 position, float direction)
        {
            return Create(ref particle, null, position, direction, Color);
        }

        public Particle Create(ref Particle particle, Vector3 position, Color color, float direction)
        {
            return Create(ref particle, null, position, direction, color);
        }

        public Particle Create(ref Particle particle, Entity entity, Vector3 position, float direction, Color color)
        {
            ...
            if (ColorMode == ColorModes.Choose)
                particle.StartColor = particle.Color = Calc.Random.Choose(Colors);
            else
