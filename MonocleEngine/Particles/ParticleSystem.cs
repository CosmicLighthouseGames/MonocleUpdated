...
namespace Monocle
{
    public class ParticleSystem : Entity
    {
        ...
        public void Emit(ParticleType type, Vector3 position)
        {        
            type.Create(ref particles[nextSlot], position);
            nextSlot = (nextSlot + 1) % particles.Length;
        }

        public void Emit(ParticleType type, Vector3 position, float direction)
        {
            type.Create(ref particles[nextSlot], position, direction);
            nextSlot = (nextSlot + 1) % particles.Length;
        }

        public void Emit(ParticleType type, Vector3 position, Color color)
        {
            type.Create(ref particles[nextSlot], position, color);
            nextSlot = (nextSlot + 1) % particles.Length;
        }

        public void Emit(ParticleType type, Vector3 position, Color color, float direction)
        {
            type.Create(ref particles[nextSlot], position, color, direction);
            nextSlot = (nextSlot + 1) % particles.Length;
        }

        public void Emit(ParticleType type, int amount, Vector3 position, Vector3 positionRange)
        {
            for (int i = 0; i < amount; i++)
                Emit(type, Calc.Random.Range(position - positionRange, position + positionRange));
        }

        public void Emit(ParticleType type, int amount, Vector3 position, Vector3 positionRange, float direction)
        {
            for (int i = 0; i < amount; i++)
                Emit(type, Calc.Random.Range(position - positionRange, position + positionRange), direction);
        }

        public void Emit(ParticleType type, int amount, Vector3 position, Vector3 positionRange, Color color)
        {
            for (int i = 0; i < amount; i++)
                Emit(type, Calc.Random.Range(position - positionRange, position + positionRange), color);
        }

        public void Emit(ParticleType type, int amount, Vector3 position, Vector3 positionRange, Color color, float direction)
        {
            for (int i = 0; i < amount; i++)
                Emit(type, Calc.Random.Range(position - positionRange, position + positionRange), color, direction);
        }

        public void Emit(ParticleType type, Entity track, int amount, Vector3 position, Vector3 positionRange, float direction)
        {
            for (int i = 0; i < amount; i++)
            {
