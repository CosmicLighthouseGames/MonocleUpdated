...
namespace Monocle {
	...
	public struct EntityData {
		...
		public Vector2 GetVector(string value) {
			return GetVector(value, Vector2.Zero);
		}
		...
		public T GetEnum<T>(string value, T defValue = default(T)) {

			if (!values.ContainsKey(value) || !(values[value] is string))
				return defValue;

			return (T)Enum.Parse(typeof(T), values[value] as string);
		}
